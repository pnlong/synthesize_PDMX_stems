...
We then compare eight factorial ablation conditions in a dual-scale webMUSHRA study (content adherence and realism) and select a production recipe for the full dataset.
...
\begin{figure}[t]
  \centering
  \centerline{\includegraphics[width=\linewidth]{figures/gm_program_counts_compare.pdf}}
...
Subjective evaluation follows MUSHRA~\cite{bs1534} via webMUSHRA~\cite{schoeffler2018webmushra}, with separate content and realism scales.
...
\section{Listening study: ablation MUSHRA}
\label{sec:listening}

We conducted a formal dual-scale MUSHRA~\cite{bs1534} using webMUSHRA~\cite{schoeffler2018webmushra} over all eight conditions in Table~\ref{tab:ablations}.
The open reference was A1 (basic); the same signal also appeared as a hidden reference among the eight shuffled conditions.
Each trial presented a $\sim$10\,s stem excerpt.
Listeners completed two pages per excerpt---\emph{content adherence} and \emph{realism}---on a 0--100 scale.
The trial set comprised 2 stems $\times$ 10 listening categories (piano, drums, strings, wind, voice, mallet, organ, guitar, brass, polyphonic), for 20 excerpts and 40 rating pages.
...
% TODO: replace with aggregate_webmushra output.
...
\begin{table}[t]
\centering
\caption{Ablation MUSHRA means (0--100). \textit{Placeholder---replace after aggregation.}}
\label{tab:mushra}
\vspace{1mm}
\small
\begin{tabular}{@{}lcc@{}}
...
Using the MUSHRA-selected recipe and the locked per-category Slakh/SA3 configs from Section~\ref{ssec:tuning}, we rendered the full PDMX-derived release set.
...
In a second listening test, participants rated pairs (or MUSHRA-style sets) of generations on content relevance to the prompt and perceived audio realism, and indicated overall preference.
...
Ablation MUSHRA guided the production recipe; training SAO on sPDMX versus Slakh2100 tested downstream value of that recipe.
